...
export class LoanController {
  ...
  update = async (req, res) => {
    try {
      ...
      const loan = await this.loanService.updateLoan(
        req.params.id,
        validation.data
      );
  ...
  getReturnedLoans = async (req, res) => {
    try {
      const loans = await this.loanService.getReturnedLoans();
      res.json(loans);
    } catch (error) {
      res.status(500).json({ message: error.message });
    }
